...
# -*- coding: utf-8 -*-
"""
generate_angular_crud_multi_v10.py
----------------------------------
...
• List server-side (MatPaginator + MatSort -> envia page/size/sort/q); fallback para array simples
• Form com upload via FormData + validações
• Rotas em src/app/app.routes.ts (export NOMEADO)
• Subpastas por entidade: src/app/componentes/<entity>/...
• Models em: src/app/shared/models
• Alerts globais (Bootstrap + Signals)
• Spinner global + HTTP interceptors (loading + erro)
• Config: shared/models/config.model.ts + config.ts (baseUrl)
• Tema Material gerado em src/styles.scss (botões coloridos)
• README.md com instruções completas + endpoints c/ cURL
"""
...
/* Hover de botões um pouco mais marcado */
...
"""
    if not os.path.exists(styles_path):
        with open(styles_path, "w", encoding="utf-8") as f:
            f.write(content)
        print("[OK] styles.scss criado com tema Material e Bootstrap importado.")
    else:
        with open(styles_path, "r", encoding="utf-8") as f:
            s = f.read()
        if "@use '@angular/material' as mat;" not in s or "mat.all-component-themes" not in s:
            s = s + "\n\n/* --- v10: Material theme + Bootstrap --- */\n" + content
            with open(styles_path, "w", encoding="utf-8") as f:
                f.write(s)
            print("[OK] styles.scss atualizado com tema Material.")
...
""")
    ...
    if "withInterceptors([" not in s:
        s = re.sub(r"provideHttpClient\((.*?)\)",
                   "provideHttpClient(withInterceptors([loadingInterceptor, errorInterceptor]))", s, flags=re.S)
        changed = True
    ...
    if changed:
        ...
        print("[INFO] app.config.ts já parece configurado.")
...
""".replace("return self  # placeholder", "return self  # patched").replace("self  # patched", "self").replace("self", "self").replace("return self", f"return self")
# The above placeholder removes. We'll set proper line below by rewriting the file.
# Easier: rebuild service_ts properly
    service_ts = f\"\"\"// Auto-generated service for {entity_name}
import {{ inject, Injectable }} from '@angular/core';
import {{ HttpClient, HttpParams }} from '@angular/common/http';
import {{ Observable }} from 'rxjs';
...
"""
    ...
    # listar server-side (export nomeado)
...
""".replace("self = 0  # placeholder", "this.rows = res; this.total = res.length;")
...
def write_routes(base_dir: str, route_entries: list):
    ...
    for e in route_entries:
        routes_ts += f"  {{ path: '{e['pathList']}', loadComponent: () => import('{e['loadList']}').then(m => m.{e['listExport']}) }},\n"
        routes_ts += f"  {{ path: '{e['pathNew']}', loadComponent: () => import('{e['loadForm']}').then(m => m.{e['formExport']}) }},\n"
        routes_ts += f"  {{ path: '{e['pathEdit']}', loadComponent: () => import('{e['loadForm']}').then(m => m.{e['formExport']}) }},\n"
    if default_redirect:
...
# --------- README ---------
def write_readme(base_dir: str, route_entries: list, api_prefix: str):
    ...
    md = f"""# Projeto Angular CRUD (gerado)

Este projeto foi gerado pelo script **generate_angular_crud_multi_v10.py**.

## Recursos
- Angular 20 standalone + Angular Material + Bootstrap 5.3.8
- CRUD por entidade em `src/app/componentes/<entidade>/`
- **Paginação e sort server-side** no listar (`page`, `size`, `sort`, `q`)
- Formulários reativos com validações e **upload via FormData**
- **Alerts** globais (Bootstrap + Signals)
- **Spinner** global (overlay) + interceptores HTTP (loading + erros)
- Rotas automáticas com **export nomeado**
- Config central de API em `shared/models/config.ts`
- **Tema Material** aplicado em `src/styles.scss`
...
## Tema do Angular Material
O tema já foi configurado em `src/styles.scss` com a API nova do Material e o import do Bootstrap:
- Se preferir **prebuilt theme**, adicione em `angular.json`:
```json
"styles": [
  "src/styles.scss",
...
`src/app/app.config.ts` (já criado/patchado):
```ts
provideRouter(routes),
provideHttpClient(withInterceptors([loadingInterceptor, errorInterceptor])),
provideAnimations(),
...
src/app/
  ...
  componentes/<entidade>/
    inserir.editar.<entidade>.ts|html|css
    listar.<entidade>.ts|html|css
...
```ts
export const config = { baseUrl: 'http://10.11.94.147:4201' };
```
O gerador aceita `--prefix` para o caminho (`{api_prefix}` por padrão).  
Ex.: baseUrl + prefix + `/users` → `http://host{api_prefix}/users`.

## Endpoints da API (exemplos)

> Ajuste o `{sample}` para a sua entidade.

- **Listar** (server-side)
```
GET {{baseUrl}}{api_prefix}/{sample}s?page=0&size=15&sort=id,asc&q=texto
```

- **Buscar por id**
```
GET {{baseUrl}}{api_prefix}/{sample}s/{{id}}
```

- **Criar (JSON)**
```bash
curl -X POST "{{baseUrl}}{api_prefix}/{sample}s" \
     -H "Content-Type: application/json" \
     -d '{"campo1":"valor","campo2":123}'
```

- **Criar (FormData / upload)**
```bash
curl -X POST "{{baseUrl}}{api_prefix}/{sample}s" \
     -F "campo1=valor" \
...
- **Atualizar**
```bash
curl -X PUT "{{baseUrl}}{api_prefix}/{sample}s/123" \
     -H "Content-Type: application/json" \
     -d '{"campo":"novo"}'
```
...
- **Excluir**
```bash
curl -X DELETE "{{baseUrl}}{api_prefix}/{sample}s/123"
```

**Formato de resposta esperado (server-side):**
```json
{ "items": [ ... ], "total": 123 }
```
ou
...
```
> Se a API devolver um **array simples**, o front também funciona (usa `length` como total).

**Erros & CORS**  
- `status 0`: problemas de rede/CORS → alerta automático no front.  
- 4xx/5xx: o interceptor exibe `status`, `method`, `url` e mensagem.
...
## Gerador
```bash
python generate_angular_crud_multi_v10.py --spec-dir ./entidades --base . --prefix /api
```
...
{{
  "nome": "User",
  "nome_tabela": "tb01_user",
  "pagination": true,
  "perpage": [15,25,50,100],
  "campos": [
    {{ "nome": "id", "tipo": "int", "primary_key": true, "autoincrement": true, "readonly": true }},
    {{ "nome": "no_user", "tipo": "str", "tam": 100, "obrigatorio": true, "input": "text" }},
    {{ "nome": "no_email", "tipo": "str", "tam": 150, "unico": true, "obrigatorio": true, "input": "email" }},
    {{ "nome": "no_password", "tipo": "str", "tam": 255, "obrigatorio": true, "input": "senha" }},
    {{ "nome": "co_validate", "tipo": "str", "tam": 12, "input": "text" }},
    {{ "nome": "dt_update", "tipo": "datetime", "default": "now", "input": "datetime" }},
    {{ "nome": "ic_ativo", "tipo": "int", "default": "1", "input": "number" }}
  ]
}}
```

## Dicas
- Botões brancos → confirme o **tema** no `styles.scss` (ou adicione prebuilt theme no `angular.json`).
- Paginador mostra **total correto** quando a API retorna `total`/`totalElements`.
- Alerts e Spinner já estão ativos via interceptores.
"""
    with open(project_readme, "w", encoding="utf-8") as f:
        f.write(md)
    print("[OK] README.md gerado/atualizado.")

# --------- main ---------
def main():
    parser = argparse.ArgumentParser(description="Gera Angular CRUD multi-entidades (v10: export nomeado, server-side paging/sort, tema Material, README).")
    ...
    parser.add_argument("--prefix", default="/api", help="Prefixo da API (default=/api). Ex.: /api, /v1")
    ...
    if routes:
        ...
        write_readme(base_dir, routes, api_prefix)
