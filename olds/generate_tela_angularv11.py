    ...
    <div class="row g-3">
      <ng-container *ngFor="let f of fields">
        <div class="col-12 col-md-6">
          ...
          <!-- Campos tipo texto, email, senha, number, time, date, datetime-local -->
          <mat-form-field *ngIf="['text','email','senha','number','time','datetime','date'].includes(f.input)"
                          appearance="outline" class="w-100">
            <mat-label>{{ f.label }}</mat-label>
            <input matInput
                   [id]="'fld-' + f.nome"
                   [type]="f.input === 'senha' ? 'password'
                           : f.input === 'email' ? 'email'
                           : f.input === 'number' ? 'number'
                           : f.input === 'time' ? 'time'
                           : f.input === 'date' ? 'date'
                           : f.input === 'datetime' ? 'datetime-local' : 'text'"
          ...
          <!-- TextArea -->
          <mat-form-field *ngIf="f.input === 'textArea'" appearance="outline" class="w-100">
            <mat-label>{{ f.label }}</mat-label>
            <textarea matInput
                      [id]="'fld-' + f.nome"
                      rows="3"
                      [attr.maxLength]="f.tam || null"
                      [readonly]="f.readonly || null"
                      [formControlName]="f.nome"></textarea>
            <mat-hint *ngIf="f.tam">Máx. {{ f.tam }} caracteres</mat-hint>
            <mat-error *ngIf="form.get(f.nome)?.hasError('required')">Campo obrigatório</mat-error>
            <mat-error *ngIf="form.get(f.nome)?.hasError('maxlength')">Ultrapassa o limite</mat-error>
          </mat-form-field>
          ...
          <mat-form-field *ngIf="f.input === 'combobox'" appearance="outline" class="w-100">
            <mat-label>{{ f.label }}</mat-label>
            <input matInput
                   [id]="'fld-' + f.nome"
                   [formControlName]="f.nome"
          ...
          <!-- Select com array estático -->
          <mat-form-field *ngIf="isArray(f.select)" appearance="outline" class="w-100">
            <mat-label>{{ f.label }}</mat-label>
            <mat-select [id]="'fld-' + f.nome" [formControlName]="f.nome">
              <mat-option *ngFor="let opt of f.select" [value]="opt">{{ opt }}</mat-option>
            </mat-select>
            <mat-error *ngIf="form.get(f.nome)?.hasError('required')">Campo obrigatório</mat-error>
          </mat-form-field>

          <!-- Select com fonte assíncrona -->
          <mat-form-field *ngIf="!isArray(f.select) && f.select" appearance="outline" class="w-100">
            <mat-label>{{ f.label }}</mat-label>
            <mat-select [id]="'fld-' + f.nome" [formControlName]="f.nome">
              <mat-option *ngFor="let opt of (loadOptions($any(f.select)) | async)"
          ...
          <!-- Radio group (fora do mat-form-field) -->
          <div *ngIf="f.input === 'radio'">
            <label class="form-label d-block mb-1" [attr.for]="'fld-' + f.nome">{{ f.label }}</label>
            <mat-radio-group class="d-flex gap-3"
                             [id]="'fld-' + f.nome"
                             [formControlName]="f.nome">
              <mat-radio-button *ngFor="let opt of (isArray(f.select) ? f.select : ['Sim','Não'])"
                                [value]="opt">
                {{ opt }}
              </mat-radio-button>
            </mat-radio-group>
            <div class="text-danger small" *ngIf="form.get(f.nome)?.hasError('required')">
              Campo obrigatório
            </div>
          </div>

          <!-- Upload de arquivo / imagem (fora do mat-form-field) -->
          <div *ngIf="f.img || f.file">
            <label class="form-label d-block mb-1" [attr.for]="'fld-' + f.nome">{{ f.label }}</label>
            <input type="file"
                   class="form-control"
                   [id]="'fld-' + f.nome"
                   (change)="onFileChange($event, f.nome)"
                   [attr.accept]="f.img ? 'image/*'
                     : (f.file==='pdf' ? 'application/pdf'
                     : (f.file==='doc' ? '.doc,.docx'
                     : (f.file==='excel' ? '.xls,.xlsx'
                     : (f.file ? '*/*' : null))))">
            <small class="text-muted" *ngIf="f.tam">Máx. {{ f.tam }} caracteres</small>
          </div>
